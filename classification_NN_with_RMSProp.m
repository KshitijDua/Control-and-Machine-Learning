function classification_NN_RMSProp
%%%%%%% DATA %%%%%%%%%%%
...
for counter = 1:Niter
    ...
    % RMSprop update
    G_W2 = decay_rate * G_W2 + (1 - decay_rate) * ((delta2 * x').^2);
    G_W3 = decay_rate * G_W3 + (1 - decay_rate) * ((delta3 * a2').^2);
    G_W4 = decay_rate * G_W4 + (1 - decay_rate) * ((delta4 * a3').^2);
    G_b2 = decay_rate * G_b2 + (1 - decay_rate) * delta2.^2;
    G_b3 = decay_rate * G_b3 + (1 - decay_rate) * delta3.^2;
    G_b4 = decay_rate * G_b4 + (1 - decay_rate) * delta4.^2;
